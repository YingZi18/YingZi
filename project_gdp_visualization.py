...
def reconcile_countries_by_name(plot_countries,gdp_countries): #返回在世行有GDP数据的绘图库国家代码字典，以及没有世行GDP数据的国家代码集合
	...
	Dict1={}
	Set=set()
	Tup=(Dict1,Set)#将dict和set合并成元组格式
	...
	for CODe in plot_countries: #所有国家代码减去字典中的为无GDP记录的国家
		if CODe not in Dict1:
			Set.add(CODe)
	return Tup#返回有GDP数据和无GDP数据国家代码
...
def build_map_dict_by_name(gdpinfo,plot_countries, year):
	...
	Set3=set()
	Dict2={}
	Tup2=reconcile_countries_by_name(plot_countries,read_csv_as_nested_dict("isp_gdp.csv","Country Code",",",'"'))

	with open(gdpinfo["gdpfile"],"rt") as csvfile:
		reader=csv.DictReader(csvfile,delimiter=gdpinfo["separator"],quotechar=gdpinfo["quote"])
		for cow in reader:
			for Code in plot_countries:
				country=plot_countries[Code]#国名对应代码数据
				if country==cow[gdpinfo["country_name"]] and cow[year]!="": #该年有记录的就加入字典
					Dict2[Code]=math.log10(float(cow[year]))#GDP结果转换为以10为基数的对数格式
...
def render_world_map(gdpinfo, plot_countries, year, map_file): #将具体某年世界各国的GDP数据(包括缺少GDP数据以及只是在该年缺少GDP数据的国家)以地图形式可视化
	...
	worldmap_chart.add(year,build_map_dict_by_name(gdpinfo,plot_countries, year)[0]) #画加入有GDP的国家
	worldmap_chart.add("missing from world bank",build_map_dict_by_name(gdpinfo,plot_countries, year)[1]) #完加入全没有GDP记录的国家
	worldmap_chart.add("no data at this year",build_map_dict_by_name(gdpinfo,plot_countries, year)[2]) #加入没有某特定年记录的国家
	worldmap_chart.render_to_file(map_file)#以map_file形式输出文件 	
